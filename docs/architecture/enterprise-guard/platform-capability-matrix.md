...
当前仓库已为 14 个平台声明 Hook 配置投影，但“可以写入 Hook 配置”不等于“企业级 HARD 阻断已验证”。Claude Code 已完成可重复项目级 Spike，并由 [覆盖能力报告](platform-coverage.md) 发布为唯一的项目级强制阻断平台；其他平台仍不属于强制阻断覆盖。
...
当前代码会在项目范围内合并受管 Router Hook，`doctor` 检查 Router runtime、重复与遗留 Hook，卸载只移除自身管理的条目。该流程的验收口令是：`install / update / doctor / uninstall` 都必须在目标平台版本上执行并记录结果。
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `claude`         | `PreToolUse` 在工具调用前提供 `cwd`、`tool_name`、`tool_input`、`tool_use_id`。                                                      | 官方文档支持 deny/ask/defer，但本基线未对多 Hook 顺序、超时和 Comet 产物执行版本固定 Probe。                                                   | `.claude/settings.local.json` 的 `PreToolUse`。                                              | [Claude Code Hooks](https://code.claude.com/docs/en/hooks) 为文档证据；项目级候选 + CI 兜底。                                                     |
